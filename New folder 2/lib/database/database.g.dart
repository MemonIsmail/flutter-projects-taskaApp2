    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `UserEntity` (`id` TEXT NOT NULL, `email` TEXT NOT NULL, `userName` TEXT NOT NULL, `password` TEXT NOT NULL, PRIMARY KEY (`id`))');
        await database.execute(
...
class _$TaskDao extends TaskDao {
  ...
  Future<void> deleteTask(int id) async {
    await _queryAdapter
        .queryNoReturn('DELETE FROM TaskEntity WHERE id = ?1', arguments: [id]);
  }
  ...
  @override
  Future<List<TaskEntity>> getTaskById(int userId) async {
    return _queryAdapter.queryList('SELECT * FROM TaskEntity WHERE userId = ?1',
        mapper: (Map<String, Object?> row) => TaskEntity(
            id: row['id'] as int?,
            title: row['title'] as String,
            dueDate: row['dueDate'] as String,
