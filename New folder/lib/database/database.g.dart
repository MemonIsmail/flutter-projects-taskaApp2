    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `UserEntity` (`email` TEXT NOT NULL, `userName` TEXT NOT NULL, `password` TEXT NOT NULL, `role` TEXT NOT NULL, PRIMARY KEY (`email`))');
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `TaskEntity` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `title` TEXT NOT NULL, `dueDate` TEXT NOT NULL, `priority` TEXT NOT NULL, `status` TEXT NOT NULL, `assignee` TEXT NOT NULL, `description` TEXT NOT NULL, `email` TEXT NOT NULL)');
...
class _$TaskDao extends TaskDao {
  ...
  Future<void> updateTask(
    ...
    await _queryAdapter.queryNoReturn(
        'UPDATE TaskEntity SET status = ?2, priority = ?3, assignee = ?4 WHERE id = ?1',
        arguments: [id, status, priority, assignee]);
  }
