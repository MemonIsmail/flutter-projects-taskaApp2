...
class _$TaskDao extends TaskDao {
  ...
  @override
  Future<List<TaskEntity>> getTaskByAssignee(String name) async {
    return _queryAdapter.queryList(
        'SELECT * FROM TaskEntity WHERE assignee =?1',
        mapper: (Map<String, Object?> row) => TaskEntity(
            title: row['title'] as String,
            dueDate: row['dueDate'] as String,
            priority: row['priority'] as String,
  ...
  Future<void> deleteTask(String title) async {
    await _queryAdapter.queryNoReturn('DELETE FROM TaskEntity WHERE title = ?1',
        arguments: [title]);
  }
