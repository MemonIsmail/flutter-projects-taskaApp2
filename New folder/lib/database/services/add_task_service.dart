...
class AddTaskService {
  ...
  Future<int> addTask(TaskDetailsModel taskModel) async {
    final database = await AppDBClient().initializeDB();
    final TaskEntity task = taskModel.taskToEntity();
    final int newId = await database.taskDao.addTask(task);
    return newId;
  }

  Future<void> updateTask(int id, String status, String priority, String assignee) async {
    final database = await AppDBClient().initializeDB();
    await database.taskDao.updateTask(id, status, priority, assignee);
  }
